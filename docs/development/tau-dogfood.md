...
### 2026-09-13 — M4 vertical waveform semantic renames

- **Identity:** accepted run `0ad9fd62-ebd9-4993-b42e-903bc07cb005`; base `0d567388`; candidate `c62892415e16`; integrated change `znmxlvqr`; model `openai-codex/gpt-5.6-luna`; 25 turns. The preceding free-model attempt `0c6069da-080b-42f2-b3d8-767497341c2a` paused after one turn without a handoff or candidate.
- **Evidence/review:** exact six-file diff renamed `torso_rise` to `torso_verticality` and `hip_rise` to `hip_ankle_vertical_extent`, preserving formulas, values, cue weights, and feature inventory. Waveform/composite schema-method-config identities were bumped; old names are absent from source, tests, and the relevant plan. Candidate and integration verification passed (`mise run check`).
- **Lifecycle/cleanup:** approved and accepted normally through Tau; accepted workspace released. The empty default working copy was rebased onto the feature bookmark, the generic candidate description was replaced with a semantic commit description, and the paused predecessor's empty workspace/state was removed under the owner's dogfood override. Human acceptance unavailable; measured inference cost unavailable.
- **Finding:** using `openai-codex/gpt-5.6-luna` produced a valid handoff after the free provider repeatedly settled without one; semantic-only channel changes should version serialized waveform and composite contracts even when numerical behavior is unchanged.
