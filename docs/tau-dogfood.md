...
- **Anchor execution:** cut, analyze, and review succeeded locally for `refs/anchors/single-serve-01.mov`; output `output/anchor-review/single-serve-01/review-phases/index.html`. Five selected frames rendered (`release`, `loading`, `cocking`, `contact`, `finish`); `start`, `acceleration`, and `deceleration` were honestly unavailable. The contact source time was `3.6675s`, with cache support at `3.669167s` (+1.667 ms).
- **Lifecycle:** reviewed and accepted through Tau with focused acceptance verification; worker workspace released; default working copy rebased. Human acceptance unavailable.
...
- **Anchor gate:** user supplied 0-based source frames: start 265, release 319, loading 355, cocking 411, acceleration 426, contact 435, deceleration 449, finish 510. Saved only under ignored `output/anchor-review/single-serve-01/`. Exact decoded-frame evaluation gave 0/5 selected keyframes accepted, three manually available stages unavailable, and only contact close (+40 ms). Pre-contact errors are hundreds of milliseconds to >1.5 s, so this is a heuristic/chronology failure rather than evidence for M4.7 cadence work. No configuration changed.
